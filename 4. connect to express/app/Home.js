      ...
      return (
            ...
            <ul className="list-group">
                {this.state.movies.map(movie =>{
                    return <li className="list-group-item"><Link to={`/movie/${movie.id}`} key={movie.id} style={{display:'block',width:'100%',height:'100%}'}}><span></span>{movie.original_title}</Link></li>;
                })}
            </ul>
        </div> 
      )
